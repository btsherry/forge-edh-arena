...
**F. Grinning Ignus loops — two variants, both sorcery-speed only.** Ignus: {2}{R} body; ability "{R}, Return this to its owner's hand: Add {C}{C}{R}. **Activate only as a sorcery**" (602.5d) — so both variants run ONLY in your own main phase, empty stack, and can never win at instant speed or in response. Summoning sickness is irrelevant (no {T} in the cost).

- **F1. Birgi, God of Storytelling + Ignus (the proven kill — a seat brain has closed a real game with this line).** Loop: cast Ignus for {C}{C}{R} → Birgi's cast trigger adds {R} → pay that {R} to bounce Ignus, adding {C}{C}{R} → recast. **Exactly mana-neutral per iteration, two cards, nothing else needed.** Each iteration is one creature ETB: with Purphoros out, 2 to each opponent per loop — pre-compute iterations = ⌈highest opponent life ÷ 2⌉ (÷4 with one doubler) and stop on the last death (104.2a). Birgi's added mana explicitly does not empty as steps and phases end, so partial loops can even carry mana across the turn.
- **F2. Runaway Steam-Kin + Ignus** (when Birgi is missing). Each red cast puts a counter on Steam-Kin; remove three for {R}{R}{R}. Per iteration you are −{R} (activation) and +1 counter; every third cast refunds {R}{R}{R} — **net-neutral only across each 3-cast cycle**, so bank the counters and spend them on schedule, and verify you can float the bridging {R} for iterations 1–2 of each cycle.
- **Interaction profile (both):** every recast is a separate spell on the stack — an opponent's counterspell stops ONE iteration, not the loop (recast again if mana allows); but instant-speed removal on Ignus *while it is on the battlefield between bounces*, or killing Birgi/Steam-Kin mid-loop, ends it. Removal on the enabler is the real threat — see the priority-window protection pattern in §6.
