...
|---|---|---|---|---|---|
| BL-29 | P1 | GUI (Chat field) | **Typing in the Advisor tab's Chat field fires Forge's match hotkeys.** Forge binds T (targeting arrows), S, C, Y, N, P, L, D at `WHEN_IN_FOCUSED_WINDOW`; a JTextField does not consume the KEY_PRESSED of a typed letter, so every letter in a question also toggled the matching panel/state — the targeting overlay went dark and yield/auto-pass state could flip mid-combat. Introduced 2026-09-04 with the Chat field (v3.4). | Game 23 (2026-09-06, Ben on Selvala): "arrows aren't coming up like normal and I am having a hard time switching targets"; no GUI exception; hotkey letters present in the typed questions. | `HotkeyGuard.swallow` consumes the KEY_PRESSED of printable unmodified keys in the field (KEY_TYPED still inserts the char; Enter/Backspace/Escape/chords untouched); Escape and a send return focus to the game. Fix written during the game; gate after it. |
