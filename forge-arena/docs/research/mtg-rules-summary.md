...
- **Playing a land** is a special action, not a spell: once per turn, own turn, stack empty, sorcery timing (`115.2a`, `305.1`); it uses no stack and cannot be responded to.
...
- **Zone-return options** (`903.9`): if a commander is in the graveyard or exile at an SBA check, its owner **may** put it into the command zone as an SBA (`903.9a`) — a real choice; sometimes the graveyard is better (reanimation). If a commander would go to its owner's **hand or library**, its owner may put it in the command zone **instead** — a replacement effect that, uniquely, can apply more than once to the same event (`903.9b`, explicit exception to `614.5`).
...
| Flash | `702.8` | May be cast any time you could cast an instant — upgrades the card to instant timing. |
...
| Lifelink | `702.15` | Damage it deals also causes its controller to gain that much life, simultaneously with the damage (not a trigger — no response window). |
...
| Trample | `702.19` | After assigning lethal to all blockers, may assign excess combat damage to the defending player/planeswalker (`702.19b`); assigns everything through if blockers are gone (`702.19d`). |
...
| Cascade | `702.85` | Cast-trigger: exile from the top of the library until hitting a nonland card of lesser mana value; may cast it free; rest to bottom in random order. Resolves *before* the cascading spell (LIFO). |
...
| Phasing | `702.26` | Phases out during its controller's untap step; while phased out it's treated as nonexistent (`702.26b`) but **no zone change occurs and no enter/leave triggers fire** (`702.26d`); counters, Auras, and Equipment phase with it (`702.26g`). Phasing a permanent out dodges "destroy all" but not sacrifice already on the stack targeting nothing. |
...
| Imprint | `207.2c` | **Ability word — no rules meaning.** Marks abilities that exile a card and refer to "the exiled card" (e.g. Isochron Scepter); all behavior comes from the individual card text. |
| Monstrosity N | `701.37` | Keyword action: if the creature isn't monstrous, put N +1/+1 counters on it and it becomes monstrous — a one-time, irreversible flag ("monstrous") that other abilities key off (`701.37b`); doesn't re-fire once monstrous. |
...
Executor note on ability words: imprint, ferocious, and metalcraft have **no entries in the CR** (`207.2c`); never dispatch on the word — always parse the ability text they decorate.
