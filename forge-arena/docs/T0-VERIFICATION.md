...
## 4.4b Addendum (PR-14, 2026-07-16): `GameState.applyToGame` is NOT usable mid-live-Commander-game

Correction to §4.4/#1's optimism: parse accepts `p0..p3` keys, but **apply** targets puzzle/dev use. `setupPlayerState` begins by clearing EVERY zone of the player — command zone included — and clearing the commander list; without `IsCommander` markers in the state text the player's commanders are simply gone, and the player's `commanderEffect` field is left pointing at an effect card that is no longer in any zone. `GameCopier.makeCopy` then throws (`Player.copyCommandersToSnapshot` → "Couldn't map Commander Effect"). Observed empirically in `SimHandleValidationTest` development; battlefield text application also misbehaved in the same runs. **Consequences:** (a) golden-test/goldfish board setup is done PROGRAMMATICALLY on the game thread (move the real commander from the command zone via `GameAction.moveToPlay`, create support cards via `Card.fromPaperCard` + `moveToPlay` with triggers suppressed, attach with `attachToEntity`, then `checkStateEffects(true)` — see the ValidationProbe pattern); (b) the v3.3 dossier goldfish must use the same pattern; (c) if scripted-text states are ever wanted mid-game, GameState needs upstream repairs (preserve commander state, or honor partial states). `ArenaGameState` remains for parse-level use.
