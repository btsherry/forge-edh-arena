...
**A4. Legal-constraint compliance (SPEC LANDED — mtg-rules-digest-conversion.md, verified against the June 2026 CR).** The digest's 18-item checklist is PR-38's acceptance list. Highest-leverage facts: the pool is PHASE-local, not priority-local — passing priority never dumps it, only the phase ending does (CR 500.5/117.4), so the whole conversion sequence chains inside one main phase; X costs lock at announcement with no partial payment (601.2f/h) — price X against the pool BEFORE casting; "may" triggers decide at RESOLUTION (603.5 — our pendingChoice seam is the legally correct shape); loop compression must be a fixed count with a pilot-controlled stop (732.2a — our bounded products are rules-faithful); after each elimination the board changes IMMEDIATELY (800.4a, not an SBA) — re-derive opponents every iteration; never win-and-lose simultaneously (104.3f) — dig guards keep library ≥ 1 unless a draw-replacement win is active first. Rule-number corrections adopted: shortcuts = CR 732 (not 727), pool-empty = 500.5 (not 500.4), free multiplayer mulligan = 103.5c (not 903.9).
