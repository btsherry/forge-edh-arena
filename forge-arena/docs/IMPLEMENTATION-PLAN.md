...
- `forge-arena/scripts/selvala-wholedeck-ingestion-wf_87514c03-1a9.js` — Fable-side harness (Claude Code Workflow-tool script). **Lean:** ~3 Fable shards over all anchors → dedup (zero-hallucination bar) → cap 200, forced through `RECORD_SCHEMA`. No Phase-II / no separate verify — verification is deferred to the compile/goldfish gate (each record's `engine_test`). *Historical note:* the gold 200-record run used a heavier 10-wide + 3-deep + 48-verify design (~62 agents) that exhausted the Fable-5 limit on re-run; leaned to ~3 agents 2026-08-03.
