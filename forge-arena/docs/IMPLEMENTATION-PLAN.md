...
- `forge-arena/scripts/selvala-wholedeck-ingestion-wf_87514c03-1a9.js` — Fable-side harness (Claude Code Workflow-tool script). **Phase I wide (~3 shards) + Phase II deep** (richest anchors — the 3-4 card chains) → dedup (zero-hallucination bar) → cap 200, forced through `RECORD_SCHEMA`. **The 48-agent adversarial-verify phase is removed** — verification is deferred to the compile/goldfish gate (each record's `engine_test`). ~6 Fable agents. *Historical note:* the gold run used 10-wide + 3-deep + **48-verify** (~62 agents); the verify pass exhausted the Fable-5 limit on re-run and was cut 2026-08-03 (Phase II kept — the deep pass is where the best chains come from).
