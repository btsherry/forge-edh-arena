...
| `runner/logs/control/voice.json` | `voice_runner.py --mute/--unmute` (GUI button later) | `voice_runner.py` (enabled=false → silence) | cleared at stop. |
| `runner/logs/voice-0.{log,jsonl}`, `runner/logs/cache/voice/*.wav` | `voice_runner.py` | humans (what was spoken/dropped), the cache saves live characters | log archived at stop; cache persists per install. |
