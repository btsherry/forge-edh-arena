...
- **v3.3.3 SHIPPED** (BL-17 closed): `build-light-package.sh --force` from `5f72c0a8714` (322M, 49329 files; no `dossier/.cache`, no `creator` strings, `react-autopass.py`/`replay.py` absent, `run_advisor.sh` present) → `forge-light-llm-20260904.tar.gz` 118634974 bytes, sha256 `56bb675dc6aaa9b5997634490f1fb18a6f7fa6a813bb182c64931673073bb12f` → REST-R2 PUT dated + `-latest` (HTTP 200 ×2), both public URLs hash-verified byte-exact. Remaining open in BUG-LOG: BL-04, BL-06 (by design), watch items W-1..W-8. Next (Ben, 10:35): advisor ask field + Send button; automatic teardown when the match is fully finished; table talk dropped.
- **Follow-on features (Ben, 10:35 — table talk dropped):** (1) Ask the advisor — `VAdvisor` field + button → `AiControlFile.askAdvisor` (`logs/control/ask/ask-<millis>-<serial>.json`, atomic, sanitised, 500 chars) → `advisor_runner._handle_asks` (numeric order, delete on pickup = the panel's "sent" signal, answered before the pause gate, `[tN · you]`/`[tN · advisor]` in the stream, record kind `ask`). (2) Auto-teardown — `scripts/arena-autostop.sh` sleep-loop watcher (engine `gameOver` or GUI PID gone → linger → exec `arena-stop.sh`; already-stopped tables left alone), `arena-play.sh --linger N` (600 human / 60 all-AI) and `--no-autostop`, `arena-stop` treats the watcher as an arena process, packager ships it. Tests: `AdvisorAskFileTest` 5 (default group), `test_advisor_ask.py` 8, `test_autostop.py` 6; Python suite 205 OK. Docs: README, INTERACTIVE-ARENA notes 73–74, PATCH-NOTES "Unreleased — after v3.3.3", INVENTORY. Not yet exercised in a live game.
- **Game 22** (human: Ben on Selvala + Urza, Giada, Purphoros; 10:52–14:29, launched from `08a1778cd7e` binaries; Ben on an outage bridge, so a slow game): **Ben won at turn 28**, all three AI seats eliminated (Giada t24, Purphoros t26, Urza t28); rated human+advisor 1041→1087, opus 959→936. 238 decisions, all model, zero punts; latency p50 4.2 s / p90 13.4 s / max 25.0 s; no REFUSED, no engine errors. Both new windows live on a human table: COLOR (Urza t19 chose Blue with a reason) and TRIGGER_ORDER (Giada t24 and t27 ordered Mana Vault's untap ahead of The One Ring's burden, with a reason). Urza's play was the highlight: Chrome Mox imprinting Swan Song for a blue source, Isochron Scepter imprinting Mana Drain, then Scepter-Drains on Giada (t8) and Drannith Magistrate (t12) with the Drain mana banked and spent. Eight deviation-log lines, one of them W-9 (the payer, not the seat, chose which artifact Urza's ability tapped). Stock surfaces: `chooseColor` ×9 for Urza's Moxen during payment (by design), `chooseColors(1-1)` ×2 for Purphoros (W-10), `chooseSomeType(Creature)` ×2 (BL-04 class). **Both new features validated live:** the Chat field was used six times (answers in 4.5–15.1 s, all six answered, exchanges in the stream and `ask` records in `advisor-0.jsonl`); the auto-stop watcher saw `gameOver` at 14:29:47, lingered the 600 s human default and ran `arena-stop.sh` at 14:39:47 — 20 files archived, game rated, mailbox/control/pids cleared, no leftover process. Its summary line said `runners=1 gui=up` and "1 arena process not covered by a PID file": both were the session's own monitoring shell, whose command line contained the marker strings (a `pgrep -f` self-match, the documented observer caveat), not a real leftover.
- **v3.4 SHIPPED** (Ben, ~15:00: "cut and ship the new build as 3.4 … make the README shorter and stronger"): README 445/3676 → 375 lines/3059 words with an independent (Opus) lost-content review applied; PATCH-NOTES v3.4; INTERACTIVE-ARENA + INVENTORY aligned; release commit `47bf4a810c2` pushed BEFORE the build so the stamp names it. Stock `build-light-package.sh --force` (322M, 49330 files) → negative checks clean (no `.cache`, no creator payloads, retired files absent, autostop + run_advisor + linger 120 + VAdvisor Chat in the shipped jar, PATCH-NOTES v3.4 at root) → `forge-light-llm-20260904b.tar.gz` 118640620 bytes, sha256 `9f955a7a86cf2e7a1c3898e6f93a21ab08256a054c28f3147bb56e91158f2862` → REST-R2 PUT dated + `-latest` (HTTP 200 ×2), both public URLs hash-verified byte-exact. v3.3.3 stays at its dated key `…-20260904.tar.gz`.
- **v3.4 refreshed same-version** (Ben: "update the cloudflare download link in the readme"): the package README now carries the `-latest` and dated download URLs (no README of ours ever had one); commit `a48f27dac9f`, rebuilt from it, `forge-light-llm-20260904b.tar.gz` re-uploaded + `-latest` — 118641057 bytes, sha256 `d89f44cb964de008c10da375456af51964c44e71c54d03f0a50b35f4d2cd8745` (supersedes `9f955a7a…`), both hash-verified.
- **Game 23** (2026-09-06, human: Ben on Selvala vs the three random bracket-3 decks Syr Gwyn / Thalia and The Gitrog Monster / Pantlaza, drawn by `scripts/sample-decks.py` and ingested with DeckCheck primers): **Ben won at turn 37** (human+advisor 1087→1123). 336 decisions, 333 model + 3 memo, zero punts, latency p50 ≈4.4 s. The newcomers played their primers (Gwyn's Sigarda's Aid → Puresteel → Buster Sword sequencing; Thalia's Path on the Hydra timed around Dosan; Pantlaza's Selvala float + discover chain and a Deflecting Swat of Ben's Song of the Dryads onto Saryth). Two defects found and fixed the same day: **BL-29** the Chat field fired Forge's window-level hotkeys while typing (Ben: "arrows aren't coming up like normal"; one question at t2 with odd counts of t/y toggled the targeting overlay and an auto-yield) → `HotkeyGuard` + focus release; **BL-30** a seat's own modal trigger with a targeted mode lost its target (`prepareTriggerViaSeat` returned right after `makeChoices`; Kogla and Yidaro's fight on Syr Gwyn dropped as "failed to target", caught by the seat's deviation log) → the chosen mode chain is aimed through the seat like any other own trigger; `ModalTriggerAimTest` (Kogla fight, Charming Prince exile, silent-brain stock fallback). Auto-stop fired on its own at +120 s. Also found on the way: the ingest lint missed meld cards (Hanweir Battlements) and DeckCheck returns Archidekt imports as plain strings — both fixed in `d36ece4d963`.
