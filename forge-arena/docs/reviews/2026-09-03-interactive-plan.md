...
## Live validation — game 19 (2026-09-03 evening, all-AI: Giada / Sheoldred / Selvala / Sythis, opus/medium, 90 s)

Launch on the new build: engine verified all four decks at 100 real cards (`launch-status.json` ok), four heartbeats and `brainAlive: true`, PID files written, per-game decision log with `game.jsonl` symlinked, legacy log set aside. Through turn 44: zero punts, zero lost windows, model latency p50 4.5 s / p95 15 s, no fizzles, target losses or trigger declines.

Seen working live:
- **Item 4**: Sheoldred's brain picked Fleshbag Marauder short of mana; the engine refused; the next window omitted the card and the brain's reasoning quoted the note ("Caverns only counts as 1 payable here; Fleshbag unaffordable. Pass"). No re-pick.
- **Item 16 measurement** (from `gui.out`, engine consultations not brain windows — de-duplicate per window before deciding): `chooseColor` fires constantly (any-color mana picks), `chooseCardsForConvokeOrImprovise` is consulted repeatedly per window by planning scans, `orderSimultaneousSa` fires every turn for the sacrifice deck (batches of 2–22), `assignCombatDamage` once on a triple block. Colors and trigger ordering are the first candidates to open.

New finding (not in the 16 items): **life-for-mana statics are beyond the stock payer's search.** With K'rrik, Son of Yawgmoth out, the brain planned Mikaeus, the Unhallowed for three mana plus six life (then Walking Ballista for infinite damage). Forge's AI payer supports `PayLifeInsteadOf:B` only when no mana source is left for the black pip; it spends the sources on the black pips first, has nothing for the generic {3}, and reports unpayable. It never tries "sources on generic, life on black". The arena's affordability guard therefore refused a legal, game-winning cast, and the deviation log recorded exactly why. Options: (a) an arena-side payment plan for `PayLifeInsteadOf` (reserve sources for generic, offer life for colored — a `forge-ai` patch under the FULL gate), or (b) expose the choice to the seat as an explicit variant option ("Mikaeus [pay 6 life for {B}{B}{B}]"). Backlog, high value for the mono-black deck.

**Result.** Sheoldred won at turn 46 (placement Sheoldred, Sythis, Giada, Selvala). 569 decisions: 540 model, 18 cycle replays, 7 memo, 4 punts. Model latency p50 4.5 s / p95 14.8 s. Zero TARGETLOSS / SA-SWAP / trigger declines / internal errors / absent brains; two correct refusals (item 4 read and acted on live); one legitimate dead-target fizzle. Seat 1 wedged once (three hung calls on a resumed session): each punted on time at 72 s, none memoized (item 3), fresh session in 4 s with "68s left for the decision" (item 2). End invariant: the two seats alive at the end read 100/100; the two eliminated seats read 0, which is CR 800.4a (a leaving player's objects leave the game) — the check now labels eliminated seats instead of calling that a vanish. Teardown by PID files left zero processes. Ratings voided the game (wedge rule). Surface consultations: chooseColor 27, convoke 9 (repeated per window), orderSimultaneousSa 6 (batches up to 22), assignCombatDamage 1.

Small follow-ups done after the game: the `REFUSED` stderr line carries needed/payableNow; eliminated seats are labelled in the card check; engine fixtures are normalized so a green gate leaves the tree clean.
