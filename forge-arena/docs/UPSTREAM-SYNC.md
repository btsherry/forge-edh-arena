...
| Audio decode (BL-56) | `forge-gui-desktop/.../sound/AudioClip.java` | Keep `getAudioClips` decoding to **16-bit** (`Converter.convertFrom(...).withTargetFormat(DECODE_FORMAT)`). If upstream replaced the converter or the loader, port the intent: the effects must reach Java Sound as 16-bit signed PCM, never 8-bit. If upstream fixed it themselves, drop ours. **Arbiter: `AudioDecodeFormatTest`** (in the FULL gate; decodes four assets with Forge's loader and asserts 16-bit signed PCM — no audio device, per Ben's rule that no lasting test may need the loopback). The loopback probe under `forge-arena/scripts/research/sound/` is optional manual research if the sound changes character after a sync (`SEQ=burst … clip afplay`: 0 impulses and a peak near 0.35 through `clip`, like `afplay`; 8-bit showed 2–6 impulses and 0.73–0.83). |
