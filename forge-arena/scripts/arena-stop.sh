...
ours() { ps -p "$1" -o command= 2>/dev/null | grep -qE "seat_runner\.py|advisor_runner\.py|voice_runner\.py|GuiPilotMatch|run_table\.sh|run_advisor\.sh|run_voice\.sh|arena-autostop\.sh"; }
skipped=0
if [ -d "$PIDS" ] && ls "$PIDS"/*.pid >/dev/null 2>&1; then
  ...
  left=$(pgrep -f "seat_runner.py --seat|advisor_runner.py|voice_runner.py|GuiPilotMatch|arena-autostop.sh" | wc -l | tr -d ' ')
  ...
  pkill -f "$ROOT/runner/run_voice.sh" 2>/dev/null
  pkill -f "$ROOT/runner/voice_runner.py" 2>/dev/null
  sleep 1
fi
...
if [ "$have_logs" = 1 ]; then
  ...
  mv "$LOGS"/seat-*.log "$LOGS"/seat-*.jsonl "$LOGS"/seat-*.usage.json \
     ...
     "$LOGS"/voice-0.log "$LOGS"/voice-0.jsonl "$LOGS"/voice_runner.out \
